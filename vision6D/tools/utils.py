...
def load_trimesh(meshpath):
    ...
    mesh = trimesh.Trimesh(vertices=meshobj.vertices.T, faces=meshobj.triangles.T,
                           process=False)  # mesh.vertices = meshobj.vertices.T.astype(np.float32) # mesh.faces = meshobj.triangles.T.astype(np.float32)
...
def writemesh(meshpath, output_path, mesh, mirror=False, suffix=''):
    ...
    meshobj.vertices = mesh.vertices.T / meshobj.sz.reshape((-1, 1)) if mesh.vertices.shape[
                                                                            1] == 3 else mesh.vertices / meshobj.sz.reshape(
        (-1, 1))
...
def create_2d_3d_pairs(color_mask: np.ndarray, vertices: pv.pyvista_ndarray, binary_mask: np.ndarray = None):
    if binary_mask is None:
        ...
        assert (binary_mask == (
                    0.3 * color_mask[..., :1] + 0.59 * color_mask[..., 1:2] + 0.11 * color_mask[..., 2:]).astype(
            "bool").astype('uint8')).all()
...
def solve_epnp_cv2(pts2d, pts3d, camera_intrinsics):
    ...
    if pts2d.shape[0] > 4:
        ...
        success, rotation_vector, translation_vector, inliers = cv2.solvePnPRansac(pts3d, pts2d, camera_intrinsics,
...
def get_actor_user_matrix(mesh_model):
    ...
    matrix = compute_transformation_matrix(mesh_model.source_obj.vertices,
                                           transform_vertices(vertices, mesh_model.actor.user_matrix))
    return matrix
...
def reset_vtk_lut(colormap):
    ...
    lut.Build()
    for i, color in enumerate(colors): lut.SetTableValue(i, color[0], color[1], color[2],
                                                         1.0)  # Last value is alpha (opacity)
    return lut


def display_warning(message):
    QtWidgets.QMessageBox.warning(QtWidgets.QMainWindow(), "vision6D", message, QtWidgets.QMessageBox.Ok,
            ...
            for attr_path, warning_message in attributes_with_messages:
                try:
                    ...
                    if attr_value is None:
                        display_warning(warning_message + f'\n{attr_path} : {attr_value}')
                        return
...
def get_bbox_from_mask(mask):
    """从一个二值掩码图中计算边界框 [x_min, y_min, width, height]。"""
